...
### Separate bars for each model 
    ...
    plt.show()
    #plt.savefig('/scratch/cburton/scratch/ISIMIP_PAPER/SeptUpdates/OnePlot_Bars.png')
...
#### Error bars for mean, min, max, WITHOUT observations 
    ...
    # setup the dataframe
        ...
        ax = plt.gca()
        plt.bar(x='Exps', height='Mean', yerr=df[['yerr_min', 'yerr_max']].T.values, capsize=10, data=df, color=['blue', 'orange', 'lightgrey'])
        ax.yaxis.set_major_formatter(FormatStrFormatter('%.2f'))
        plt.yticks(fontsize=fontsize)
        plt.xticks(fontsize=fontsize)
        plt.title(str(key), fontsize=fontsize)
        plt.savefig('/scratch/cburton/scratch/ISIMIP_PAPER/SeptUpdates/'+str(key)+'_Models.png')
