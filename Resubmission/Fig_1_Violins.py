...
def main(): 
    ...
    #f = open('/scratch/cburton/scratch/ISIMIP_PAPER/Resubmission/Fig1%.dat','a')
...
#### Error bars for mean, min, max, WITHOUT observations 
    v = {}
    EXPS = ['RCP60_1p5','RCP60_2p0']
    for key in Region_dict:
        for model in models:
            for EXP in EXPS:
                v[model+EXP+key] = ((d[model+EXP+key] - d[model+"2010-2019"+key]) / d[model+"2010-2019"+key]) *100

        
        deg15 = [ v["HadGEM2RCP60_1p5"+key] ,  v["MIROCRCP60_1p5"+key] , v["GFDLRCP60_1p5"+key] , v["IPSLRCP60_1p5"+key] ]

        deg20 = [ v["HadGEM2RCP60_2p0"+key] ,  v["MIROCRCP60_2p0"+key] , v["GFDLRCP60_2p0"+key] , v["IPSLRCP60_2p0"+key] ]
        ...
        ax = plt.gca()
        fontsize=24
        violin_parts = plt.violinplot(data, showmedians=False,showmeans=True, showextrema=True)

        colors = ['blue', 'orange']
        for pc, color in zip(violin_parts['bodies'], colors):
            pc.set_facecolor(color)
            pc.set_edgecolor('black')
        violin_parts['cmeans'].set_colors('black')

        #plt.yticks(fontsize=fontsize)# Different y axis
        plt.yticks([-50, -25, 0, 25, 50],fontsize=fontsize)# Same y axis
        ax.yaxis.set_major_formatter(FormatStrFormatter('%.0f'))
        plt.yticks(fontsize=fontsize)# Different y axis
        plt.xticks([])
        #plt.title(str(key), fontsize=fontsize)
        plt.savefig('/scratch/cburton/scratch/ISIMIP_PAPER/Resubmission/'+str(key)+'_%V.png')
        plt.close()
...
### Separate bars for each model 
 ...
 ### Error bars for mean, min, max, WITH observations
    ...
    # setup the dataframe
        ...
        plt.title(str(key))
        #plt.show()
        plt.savefig('/scratch/cburton/scratch/ISIMIP_PAPER/Resubmission/'+str(key)+'+obs.png')       
